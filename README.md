...
## Importing Mokuro Files

Before you can read manga in the application, you need to import your Mokuro-processed files into the correct directory structure.

### File Organization

The application uses a `host-data` directory as a protective mechanism - the container only has access to this directory, preventing the reader from accessing your entire system.

**Important:** Place all your Mokuro files in the `host-data/manga-library` directory.

### Directory Structure Example

```
user@user-VM:hanabira.org_manga_reader$ tree host-data/
host-data/
├── backend-data
│   └── images
├── logs
└── manga-library
    └── ruri_rocks_vol1
        ...
        │   ├── rr_01.png
        │   ├── rr_02.png
        ├── ruri_rocks_ch_1.html
        └── ruri_rocks_ch_1.mokuro

9 directories, 42 files
user@user-VM:hanabira.org_manga_reader$ 
```

### Steps to Import

1. **Copy your Mokuro files** to `host-data/manga-library/[your-series-name]/`
   - Include the `.html` file (generated by Mokuro)
   - Include the `.mokuro` file
   - Include the image directory (containing PNG/JPG files)
   - Include the `_ocr` directory (containing JSON files)

2. **Add the series in the application:**
   - Go to **Library** → **Add Series** → **Add Chapter**
   - Click **Browse Mounted Files**
   - Navigate to your series folder
   - Select the `.html` file generated by Mokuro

The application will then be able to read your manga with full OCR text support, dictionary lookups, and flashcard functionality.
